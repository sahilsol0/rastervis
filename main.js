...
async function ddaLine(x1,y1,x2,y2) {
  ...
  newli.lastChild.classList.add('highlighted3')
  ...
  newli = document.createElement(`li`)
  newli.innerHTML = `${x.toFixed(3)}+${xinc.toFixed(3)} = ${(x + xinc).toFixed(3)}, ${y.toFixed(3)}+${yinc.toFixed(3)} = ${(y + yinc).toFixed(3)} | <b class = 'highlighted1'>(${Math.round(x+xinc)},${Math.round(y+yinc)})</b>`
  ol.appendChild(newli)
...
points.addEventListener('change',async(e)=>{
  grid.childNodes.forEach((node) => {
    node.style.background = color[3]
  })
